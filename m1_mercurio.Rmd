...
# Análisis de datos y preguntas base
...
Como el valor p es menor que $\alpha$ y $z_0$ es mayor que $z*$, por lo que no se rechaza $H_0$.
...
[]: <> (HACER ANÁLISIS DE INTERVALOS DE CONFIANZA: recurrir a utilizar la Z para el teorema central del límite. Hacer comparaciones múltiples, que vimos en el ANOVA, en tiempos de secado, sabíamos que había una media que es diferente pero no sabíamos cual. Con Tukey hace las comparaciones, NO TIENE CASO TUKEY ).
